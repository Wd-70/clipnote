...
export default function PlaylistEditModal({ playlist, isOpen, onClose, onUpdate }: PlaylistEditModalProps) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (!response.ok) {
        ...
        throw new Error(errorData.error || '플레이리스트 수정에 실패했습니다.')
      }

      const updatedPlaylist = await response.json()
      onUpdate(updatedPlaylist.playlist)
      setSuccess('플레이리스트가 성공적으로 수정되었습니다!')
  ...
  const isFormChanged = () => {
    return formData.name !== playlist.name ||
           formData.description !== (playlist.description || '') ||
           formData.isPublic !== playlist.isPublic
  }
          ...
          >
            ...
            <form onSubmit={handleSubmit} className="p-6 space-y-4">
              ...
              <div className="bg-gray-50 dark:bg-gray-700/50 rounded-lg p-3">
                <div className="text-sm text-gray-600 dark:text-gray-400">
                  <div className="flex justify-between mb-1">
                    <span>곡 수:</span>
                    <span className="font-medium">{playlist.songCount}곡</span>
                  </div>
              ...
              <div className="flex gap-3 pt-4">
                ...
                <button
                  type="submit"
                  disabled={isLoading || !formData.name.trim() || !isFormChanged()}
